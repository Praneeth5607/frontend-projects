# 📊 Attendance Calculator

A simple web application to calculate attendance percentage and determine whether you can skip classes or need to attend more to maintain the 75% rule.

---
## 🌐 Live Demo

View the project here:

---
## 🚀 Features

* Calculate attendance percentage instantly
* Shows how many classes you can skip (if ≥ 75%)
* Shows how many classes you need to attend (if < 75%)
* Input validation for incorrect values
* Dynamic color indication based on result
* Clean and simple UI

---

## 🧠 Logic Used

* **Attendance %** = (Attended / Total) × 100
* If percentage ≥ 75 → calculate **skippable classes**
* If percentage < 75 → calculate **required classes to attend**

---

## 🛠️ Tech Stack

* HTML
* CSS
* JavaScript (DOM manipulation + logic)

---



## 📂 Project Structure

```
attendance-calculator/
│── index.html
│── style.css
│── script.js
│── README.md
```

---

## ▶️ How to Run

1. Download or clone the repository
2. Open `index.html` in your browser

---

## 📌 Future Improvements

* Add progress bar visualization
* Store previous inputs using localStorage
* Real-time calculation (without button)
* Mobile responsiveness improvements

---

## 👤 Author

Praneeth

---
